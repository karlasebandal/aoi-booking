...
app.get("/bookingListByGuest/:guestId", async (req, res) => {
  try {
    ...
    const bookings = await pool.query("SELECT bookingid, bookingdate, bookingtime, booking.status, bookingtype, service.name AS service_name FROM Booking INNER JOIN Service ON booking.serviceid = service.serviceid WHERE guestId = $1", [guestId]);
...
app.get("/payment/:guestId", async(req, res) => {
    // try {
    //     const allPayment = await pool.query(
    //         'SELECT * FROM payment');
    //     res.json(allPayment.rows);
    ...
    try {
        ...
        const bookings = await pool.query("SELECT service.name AS service_name,booking.status AS booking_status, booking.bookingtype AS booking_type, payment.depositamount AS deposit_amount, payment.totalpayment AS total_payment FROM Booking INNER JOIN Service ON Booking.serviceid = Service.serviceid LEFT JOIN Payment ON Booking.bookingid = Payment.bookingid WHERE Booking.guestid = $1;", [guestId]);
...
app.get("/Guest/login", async(req, res) => {
    ...
    app.get('/countNumGuestsPerDate', async (req, res) => {
      console.log(`im in!`)


      try {
        // SQL query to count numGuests per bookingDate
        const query = `
          SELECT
              TO_CHAR(bookingDate, 'YYYY-MM-DD') AS formattedBookingDate,
              SUM(numGuests) AS totalNumGuests
          FROM
              Booking
          GROUP BY
              bookingDate
